...
public class ConfigRuleService {
    ...
    public Object updateUserRole(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        if (userid.equals(adminUserId)) {
            ...
            List<String> roleId = Lists.transform(roleDao.getRoleList(userid, companyId), new Function<DashboardRole, String>() {
                @Nullable
                @Override
        ...
        if (userid.equals(adminUserId)) {
            ...
            List<Object> filtered = arr.stream().filter(e -> {
                ...
                switch (jo.getString("resType")) {
                    case "widget":
                        return widgetDao.checkWidgetRole(userid, jo.getLong("resId"), RolePermission.PATTERN_READ, companyId) > 0;
                    case "dataset":
                        return datasetDao.checkDatasetRole(userid, jo.getLong("resId"), RolePermission.PATTERN_READ, companyId) > 0;
                    case "board":
                        return boardDao.checkBoardRole(userid, jo.getLong("resId"), RolePermission.PATTERN_READ, companyId) > 0;
                    default:
                        return false;
                }
