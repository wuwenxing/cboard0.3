...
public class DataProviderRoleService {
    ...
    public Object query(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        if (datasetId != null) {
            if (datasetDao.checkDatasetRole(userid, datasetId, RolePermission.PATTERN_READ, companyId) > 0) {
                return proceedingJoinPoint.proceed();
            }
        } else {
            if (datasourceDao.checkDatasourceRole(userid, datasourceId, RolePermission.PATTERN_READ, companyId) > 0) {
                return proceedingJoinPoint.proceed();
            }
        }
        return null;
